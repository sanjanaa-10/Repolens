...
This guide documents how to run RepoLens in production: the intended
production topology (reverse proxy + TLS in front of a loopback-bound backend,
static frontend build), the environment variables that control a production
install, backups, and operational checks.

Per the scope described in [SECURITY.md](./SECURITY.md), RepoLens is a **local,
single-user** tool. No authentication/authorization, multi-user, OAuth, private
repos, Docker/Kubernetes, or distributed workers are implemented or planned.
This guide therefore does **not** add Docker or CI; it provides the reference
deployment for the architecture that actually exists.
...
The full backend suite passes: 249 passed / 2 skipped / 4 deselected / 1 warning,
and the frontend typecheck + production build both pass.