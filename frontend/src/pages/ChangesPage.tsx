...
import { ChangeView } from "../components/analysis/ChangeView";
  ...
  if (!repo) {
    return (
      <div className="flex h-full items-center justify-center">
        <EmptyState
          icon={
            <GitCompare className="h-8 w-8 animate-pulse text-accent-hover" />
          }
          title="Loading repository…"
          description="Fetching repository metadata before change analysis."
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-10 shrink-0 items-center gap-1.5 border-b border-line bg-ink-900 px-4 text-xs text-text-muted">
        ...
        <ChevronRight className="h-3 w-3 text-text-faint" />
        <span className="text-accent-hover">Changes</span>
      </div>
      <ChangeView repositoryId={repo.id} />
    </div>
  );
}